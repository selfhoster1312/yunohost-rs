...
pub struct ExportBookwormContainer {
    #[serde(flatten)]
    pub fields: Map<String, Value>,
}

impl ExportBookwormContainer {
    pub fn new() -> Self {
        Self { fields: Map::new() }
    }
            ...
            for (section_id, section) in &panel.sections {
                ...
                for (option_id, option) in &section.options {
                    ...
                    if let Ok(option_type) = OptionType::from_str(&option.option_type) {
                        ...
                        if option_type == OptionType::Tags {
                            // WHY THIS SPECIAL CASE AGAIN?
                            let value =
                                ConfigPanel::value_or_default(&option_id, &option, &saved_settings);
